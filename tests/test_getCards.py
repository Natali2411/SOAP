...
def test_cardId(openTestData, obj):
    param = openTestData["test_set"]
    res = obj.connectToWebService().service.getCards(shopId=param["shopId"], cardId=param["cardId"])
    assert res["countMessages"]
...
def test_limit(openTestData, obj):
    param = openTestData["test_set"]
    res = obj.connectToWebService().service.getCards(shopId=param["shopId"], limit=param["limit"])
    assert res["countMessages"]

def test_offset(openTestData, obj):
    param = openTestData["test_set"]
    res = obj.connectToWebService().service.getCards(shopId=param["shopId"], offset=param["offset"])
    assert res["countMessages"]
